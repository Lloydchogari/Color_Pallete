  ...
  return (
    <div className="min-h-screen w-screen max-w-full bg-blue-950 relative overflow-x-hidden overflow-y-auto text-white font-chillax">
      ...
      {circles.map((circle) => (
        <div
          key={circle.id}
          className="absolute rounded-full opacity-70 animate-blink"
          style={{
            top: `${circle.top}%`,
            left: `${circle.left}%`,
      ...
      {!showUploader ? (
        <div className="h-screen w-full flex flex-col items-center justify-center z-10 relative text-center bg-black/30 backdrop-blur-sm px-4">
          ...
          <p className="mb-6 text-md md:text-lg text-gray-200">Upload an image to reveal its beautiful palette</p>
          <button
        ...
        <div className="relative w-full min-h-screen flex items-center justify-center z-10 p-4 overflow-y-auto">
          <div className="backdrop-blur-lg bg-white/10 border border-white/30 shadow-2xl rounded-2xl p-6 md:p-8 w-full max-w-xl text-center">
